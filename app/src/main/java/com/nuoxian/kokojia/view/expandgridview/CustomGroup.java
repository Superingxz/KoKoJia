package com.nuoxian.kokojia.view.expandgridview;
...
public class CustomGroup extends ViewGroup {
	...
	public static final int COLUMNUM = 4;//列数
...
//		initData();
	 ...
	 */
	public void initData(final ArrayList<DragIconInfo> iconInfoList) {
	 ...
	 */
	private void initIconInfo(ArrayList<DragIconInfo> iconInfoList) {


		allInfoList.clear();
		allInfoList.addAll(iconInfoList);
		getPageInfoList();
...
//		iconInfoList.add(new DragIconInfo(7, "第4个可展开", R.mipmap.ic_launcher, DragIconInfo.CATEGORY_EXPAND, childList));
//		iconInfoList.add(new DragIconInfo(8, "第5个可展开", R.mipmap.ic_launcher, DragIconInfo.CATEGORY_EXPAND, childList));
//		iconInfoList.add(new DragIconInfo(9, "第6个可展开", R.mipmap.ic_launcher, DragIconInfo.CATEGORY_EXPAND, childList));
//		iconInfoList.add(new DragIconInfo(10, "第7个可展开", R.mipmap.ic_launcher, DragIconInfo.CATEGORY_EXPAND, childList));
//
//		return iconInfoList;
//	}
...
//			homePageInfoList.add(new DragIconInfo(CustomAboveView.MORE, "更多", R.mipmap.icon_home_more, 0, new ArrayList<DargChildInfo>()));
	...
	protected void dispatchChild(DargChildInfo childInfo) {
		...
		intent.putExtra("urlId", Integer.parseInt(childInfo.getClassId()));
		intent.putExtra("urlPage", 1);
		if(TextUtils.isEmpty(childInfo.getCourseName())){
			intent.putExtra("title", childInfo.getName());
			intent.putExtra("classify", childInfo.getClassName());
		}else{
			intent.putExtra("title",childInfo.getCourseName());
			intent.putExtra("classify", childInfo.getCourseName());
		}
	...
	public void dispatchSingle(DragIconInfo dragInfo) {
		...
		//跳转到课程列表页面
		Intent intent = new Intent(mContext, CourseListActivity.class);
		intent.putExtra("urlId",Integer.parseInt(dragInfo.getClassId()));
		intent.putExtra("urlPage",1);
		intent.putExtra("title",dragInfo.getName());
		mContext.startActivity(intent);
	}
